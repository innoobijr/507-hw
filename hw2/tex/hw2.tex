...
\HW{2}
Cameron Evans, Innocent Obi Jr.

\Due{November 10, 2023}

\begin{document}
\newcommand{\brc}[1]{\lbrace{#1}\rbrace}
\newcommand{\brck}[3][s_{T_{=}} &=]{
\begin{equation}
{#1} \lbrace{#2}\rbrace \qquad \textrm{#3}
\end{equation}
}
\maketitle
...
\brck{\brc{x}, \brc{y}, \brc{f(x)} \brc{f(y)}, \brc{g(x)}, \brc{g(f(x))} \brc{g(f(y))}, \brc{f(g(x))}, \brc{g(f(y))}}{}

Due to the first equality, we get the following merged equivalence class:
\brck[]{f(g(x)), g(f(x))}{given f(g(x)) = g(f(x)}

After the second and third equalities, we merge and get:
\brck[]{f(y), x}{given f(y) = x}
\brck[]{f(g(f(y))), x}{given f(g(f(y))) = x}
...
\brck[]{f(g(f(y))), x, f(y), g(f(x), f(g(x))}{merge. {f(g(f(y)) = f(g(x))}}

To find the the second equivalence class, we take $f(y) = x$ and get the following equalities:

\brck[]{g(f(y)), g(x)}{\text{if f(y)=x  then g(f(y)) = g(x). func congruence}}

Finally, 
\brck[]{g(f(y)), y, g(x)}{\text{if f(g(f(y)))= f(g(x)) = x = f(y) then g(x) = y. func congruence}}\\
